  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    menu.querySelectorAll('.nav-item.has-sub > .nav-sub-toggle').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        if (!mqMobile.matches) return;     // 桌機交給 CSS hover
        e.preventDefault();
    ...
    document.addEventListener('visibilitychange', function () {
      if (document.hidden) stop(); else { start(); startQuick(); }
    });
    start();
    ...
    var qcTimer = null;
    function qcGo(idx) {
      qcSlides[qcCurrent].classList.remove('is-active');
      qcCurrent = (idx + qcSlides.length) % qcSlides.length;
      qcSlides[qcCurrent].classList.add('is-active');
    }
    function startQuick() { if (qcSlides.length > 1 && !qcTimer) qcTimer = setInterval(function () { qcGo(qcCurrent + 1); }, 4500); }
    function stopQuick() { if (qcTimer) { clearInterval(qcTimer); qcTimer = null; } }
    var qcWrap = document.getElementById('quickCards');
    if (qcWrap) {
      qcWrap.addEventListener('mouseenter', stopQuick);
      qcWrap.addEventListener('mouseleave', startQuick);
    }
    startQuick();
